-- Saga Eterna - Avance de día con disputas PvP
...
begin
  ...
  -- 2. Refuerzos automáticos desde capitales a ciudades.
  for v_capital in
    select id, name, soldiers, owner_kingdom_id
    from public.territories
    where type = 'CAPITAL'
  ...
  loop
    ...
    -- Si ya hay disputa abierta sobre este territorio:
    -- - Si llega el mismo atacante, se suma al asedio.
    -- - Si el objetivo ya es suyo, se integra como refuerzo.
    select *
    into v_open_dispute
    ...
    -- PvP solo si ambos reinos tienen jugador.
    select exists (
      select 1
      from public.profiles p1
      where p1.kingdom_id = v_movement.kingdom_id
    )
    and exists (
      select 1
      from public.profiles p2
      where p2.kingdom_id = v_target.owner_kingdom_id
    )
    into v_is_pvp;

    if v_attackers_win and v_is_pvp then
      -- No se conquista automáticamente: se abre disputa.
      update public.territories
      set
        soldiers = 0,
        is_disputed = true,
    ...
    -- Si no es PvP disputable, se resuelve normal.
    update public.territories
    set
...
select 'Avance de día con disputas PvP actualizado.' as result;
