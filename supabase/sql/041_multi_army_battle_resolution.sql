-- Saga Eterna - Resolución automática de batallas multi-ejército
-- Cuando una batalla NO entra en disputa presencial:
-- - Se compara defensor vs todos los atacantes que llegan al mismo territorio.
-- - Gana el bando con más soldados.
-- - Empate en el primer puesto = el defensor resiste.
-- - Supervivientes = fuerza ganadora - segunda fuerza más alta.
-- - Si gana atacante PvP, se abre disputa presencial como antes.
...
begin
  ...
  loop
    ...
    if v_open_dispute.id is not null
      ...
      update public.territory_dispute_attackers
      set
        soldiers = soldiers + v_movement.soldiers,
        updated_at = now()
      where dispute_id = v_open_dispute.id
        and kingdom_id = v_movement.kingdom_id;
  ...
  loop
    ...
    -- Si ya hay disputa abierta, cualquier ataque que llegue se convierte en fuerza atacante.
    if v_open_dispute.id is not null then
      ...
      loop
        if v_attacker.kingdom_id = v_target.owner_kingdom_id then
          update public.territories
          set
            soldiers = coalesce(soldiers, 0) + v_attacker.soldiers,
            updated_at = now()
    ...
    -- Si el objetivo ya pertenece a algún atacante, esos soldados se integran como refuerzo.
    ...
    -- Releer defensor después de posibles integraciones.
    select id, name, type, soldiers, owner_kingdom_id, is_disputed
    into v_target
    from public.territories
    where id = v_target_id
    ...
    -- Si no quedan atacantes enemigos, pasamos al siguiente territorio.
    select count(*)
    ...
    -- Calcular fuerza máxima entre defensor y todos los atacantes.
    with sides as (
    ...
    -- Si el defensor tiene la mayor fuerza, resiste.
    ...
    -- Si el ganador atacante y el defensor son ambos jugadores, se abre disputa.
    select exists (
    ...
    if v_is_pvp then
      ...
      -- Registrar todos los atacantes participantes en la disputa.
      for v_attacker in
        select kingdom_id, sum(soldiers)::integer as soldiers
        from public.troop_movements
        where status = 'IN_TRANSIT'
    ...
    -- Resolución automática normal: el atacante con más fuerza conquista.
    update public.territories
    set
      soldiers = v_remaining_soldiers,
...
select 'Resolución multi-ejército de batallas actualizada correctamente.' as result;
